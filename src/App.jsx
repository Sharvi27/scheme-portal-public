...
// Lifted out of ProfileForm so it doesn't remount on every render
...
// Reusable field label — uses the Label component consistently
...
function ProfileForm({ profile, onChange, onSearch, loading, schemesReady }) {
  ...
  return (
    <div style={{ background: 'white', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow)', padding: '28px 24px', border: '1px solid var(--border)' }}>
      <h2 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: 20, color: 'var(--navy)' }}>Your Details</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div>
          ...
          {profile.annual_income && <p style={{ fontSize: '0.72rem', color: 'var(--saffron)', marginTop: 4 }}>Auto-derived. Select manually to override (clears income).</p>}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[['', 'Not Specified'], ['delhi', '🏙️ Delhi'], ['haryana', '🟢 Haryana'], ['other', '🗺️ Other']].map(([v, l]) => (
            <button key={v} onClick={() => onChange('state', v)} style={{
              flex: 1, padding: '8px 4px', borderRadius: 'var(--radius-sm)',
              border: '1.5px solid', fontSize: '0.82rem', fontWeight: 500, cursor: 'pointer',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 16 }}>
        ...
        <BoolToggle fieldKey="is_widow" label="Widow?" value={profile.is_widow} onChange={onChange} />
        <BoolToggle fieldKey="is_disabled" label="Disabled?" value={profile.is_disabled} onChange={onChange} />
      </div>
...
function SchemeCard({ scheme, onClick }) {
  return (
    ...
    >
      ...
      {scheme.scheme_type && (
        <span style={{ display: 'inline-block', marginTop: 10, padding: '2px 10px', borderRadius: 20, fontSize: '0.72rem', fontWeight: 600, background: '#f1f5f9', color: 'var(--text-muted)', textTransform: 'uppercase' }}>
          {scheme.scheme_type}
        </span>
      )}
      <div style={{ marginTop: 10, fontSize: '0.8rem', color: 'var(--saffron)', fontWeight: 600 }}>
        View details →
      </div>
    </div>
  )
...
export default function App() {
  ...
  // Single query: fetch schemes with eligibility joined — no double round-trip
  useEffect(() => {
    async function load() {
  ...
  // No fake setTimeout — matching is instant pure JS
  ...
  // Memoised — only recomputes when results or filterBody changes
  const displayed = useMemo(() => {
    if (!results) return []
    if (filterBody === 'all') return results
    return results.filter(s => s.issuing_body === filterBody)
  }, [results, filterBody])
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 10 }}>
                ...
                {results.length > 0 && (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {[['all', 'All'], ['delhi', '🏙️ Delhi'], ['central', '🇮🇳 Central'], ['haryana', '🟢 Haryana']].map(([b, label]) => (
              ...
              {displayed.length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
                  {displayed.map((s, i) => (
                    // animationDelay is on the card itself, not a wrapper, so it actually works
                    <SchemeCard key={s.id} scheme={s} onClick={() => setSelected(s)} style={{ animationDelay: `${i * 0.04}s` }} />
                  ))}
                </div>
              )}
            </>
